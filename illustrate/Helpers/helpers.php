...
if (!function_exists('serialize')) {
    ...
    {
        \illustrate\Closure\SerializableClosure::enterContext();
        \illustrate\Closure\SerializableClosure::wrapClosures($data);
        $data = \serialize($data);
        \illustrate\Closure\SerializableClosure::exitContext();
        return $data;
    }
}
if (!function_exists('unserialize')) {
     ...
     * @return mixed
     */
    function unserialize($data, array $options = null): mixed
    {
        \illustrate\Closure\SerializableClosure::enterContext();
        $data = ($options === null || \PHP_MAJOR_VERSION < 7)
            ? \unserialize($data)
            : \unserialize($data, $options);
        \illustrate\Closure\SerializableClosure::unwrapClosures($data);
        \illustrate\Closure\SerializableClosure::exitContext();
        return $data;
    }
}
...
if (!function_exists('isValidJsonType')) {
    function isValidJsonType(string $type): bool
    {
        if (isset(JSON_SUBTYPES[$type])) {
            return true;
        }

        return in_array($type, JSON_TYPES, true);
    }
}
if (!function_exists('getJsonSuperType')) {
    function getJsonSuperType(string $type): ?string
    {
        return JSON_SUBTYPES[$type] ?? null;
    }
}
if (!function_exists('getJsonType')) {
    function getJsonType($value, bool $use_subtypes = true): ?string
    {
        $type = PHP_TYPE_MAP[gettype($value)] ?? null;
        if ($type === null) {
            return null;
        } elseif ($type === 'array') {
            return isIndexedArray($value) ? 'array' : null;
        }

        if ($use_subtypes) {
            if ($type === 'number' && isMultipleOf($value, 1)) {
                return 'integer';
            }
        } elseif ($type === 'integer') {
            return 'number';
...
if (!function_exists('jsonTypeMatches')) {
    function jsonTypeMatches(string $type, $allowed): bool
    {
        if (!$allowed) {
            return false;
        }
        ...
        if (is_string($allowed)) {
            ...
            return $allowed === getJsonSuperType($type);
        }
...
if (!function_exists('valueIsOfJsonType')) {
    function valueIsOfJsonType($value, $type): bool
    {
        $t = getJsonType($value);
        if ($t === null) {
            return false;
...
if (!function_exists('convertAssocArrayToObject')) {
    function convertAssocArrayToObject($schema)
    {
...
if (!function_exists('equals')) {
    function equals($a, $b): bool
    {
        ...
        $type = getJsonType($a, false);
        if ($type === null || $type !== getJsonType($b, false)) {
            return false;
        }
...
if (!function_exists('isMultipleOf')) {
    function isMultipleOf($number, $divisor, int $scale = 14): bool
    {
        static $bcMath = null;
        if ($bcMath === null) {
...
if (!function_exists('isValidPattern')) {
...
if (!function_exists('patternToRegex')) {
...
if (!function_exists('app')) {
    function app(string $object = null): Application
    {
        return Application::$app;
    }
}
if (!function_exists('redirect')) {
    function redirect($url): \illustrate\Response
    {
        $redirect = app()->make(\illustrate\Response::class);
        return $redirect->redirect($url);
    }
}
if (!function_exists('request')){
    function request():\illustrate\Request
    {
    }
}